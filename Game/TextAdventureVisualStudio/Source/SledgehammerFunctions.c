...
void Brick_Use(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	Item* sledgehammer; /* The brick in the player's inventory */
	...
	else
	{
		...
		gameState->inventory = ItemList_Remove(gameState->inventory, sledgehammer);
...
Item* Sledgehammer_Build()
{
	...
	return Item_Create("sledgehammer", "A sturdy sledgehammer that feels pretty heavy", true, Brick_Use, Brick_Take, NULL);
}