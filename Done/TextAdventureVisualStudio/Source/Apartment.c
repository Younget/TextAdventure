/******************************************************************************
filename    Apartment.c
author      Esther Strathy

Brief Description:
Creates the final room, Max Hemlock's apartment
...
{
	Room *Apartment; /* the new room object */
	
	/* allocate the memory for the new object */
	Apartment = (Room*)malloc(sizeof(Room));
	if (Apartment == NULL)
	{
	...
	Room_SetDescription(Apartment, description);

	/* initialize the new room's list pointers to NULL */
	Apartment->itemList = NULL;
	Apartment->roomExitHead = NULL;
