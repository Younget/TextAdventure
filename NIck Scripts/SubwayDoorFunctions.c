...
void SubwayDoor_Use(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	if (subwayticket != NULL)
	{
		...
		printf("With a loud bang, The gate before you falls flat onto the ground. LOL you almost died, be careful going forward.");
		return;
	}

	else
	{
		printf("You cannot enter this barricaded door without excessive force, which your body seemingly lacks. Weakling\n");
		return;
	}
}
