...
void Password_Read(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	if (context != CommandContext_Item_Inventory)
	{
		...
		printf("You don't have it moron, better find that password kiddo.\n");
		return;
	}
	else
	{
		printf("Upon inspecting the password, you determine that the password is 'immortal'.\nHa Ha I bet this is the same type of person that accepts all terms and conditions without looking through it.\n");
	}
}
...
Item* Password_Build()
{
	...
	return Item_Create("password", "A password crudely written on a shriveled lump of paper with a red stain on the back", true, NULL, Password_Take, NULL, Password_Read, NULL, NULL);
}