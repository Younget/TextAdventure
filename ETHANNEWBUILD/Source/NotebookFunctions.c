...
void Notebook_Read(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	else
	{
		...
		if (gameState->farthestRoom >= 1) {
			printf("There is vegetaion everywhere. It seems like this park has been abandoned for years. There are some statues that are covered by moss.");
		}
		else return;
...
Item* Notebook_Build()
{
	...
	return Item_Create("notebook", "A notebook", "text", true, NULL, Notebook_Take, NULL, Notebook_Read, NULL, NULL);
}