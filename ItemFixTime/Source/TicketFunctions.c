...
void Ticket_Take(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	printf("You pick up the ticket. This might come in handy later.\n");
}
...
void Ticket_Use(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	if (gameState->currentRoomIndex != 5)
	{
		...
		printf("There's no use for the ticket here.\n");
		return;
	}
...
Item* Ticket_Build()
{
	...
	return Item_Create("ticket", "A subway ticket", true, Ticket_Use, Ticket_Take, NULL, NULL, NULL, NULL);
}